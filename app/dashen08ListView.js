...
export default class dashen08ListView extends Component {
  constructor(props) {
    ...
    // dataBlob：包含ListView所需的所有数据（section header 和 rows），在ListView渲染数据时，使用getSectionData 和 getRowData 来渲染每一行数据。
    // sectionIDs：sectionIDs 用于标识每组section
    // rowIDs：rowIDs 用于描述每个 section 里的每行数据的位置及是否需要渲染。
